# https://codecombat.com/play/level/kelvintaph-defiler?
...
def commandTroops():
    for index, friend in enumerate(hero.findFriends()):
        if (step == 0 and friend.pos.y > 60 and (friend.type != 'archer' or hero.time > 3.6)):
...
def CommandPaladin(soldier):
    if step < 4:
        if (soldier.canCast("heal") and hero.time > 3.0):
            hero.command(soldier, "cast", "heal", soldier)
        elif hero.time > 2.8 and soldier.health < soldier.maxHealth * 0.7:
...
while True:
    if (hero.time > 6 and hero.time < 12):
        step = 1
    elif (hero.time > 10 and hero.time < 17):
        step = 2
    elif (hero.time > 15 and hero.time < 29):
        step = 3
    elif (hero.time > 27 and hero.time < 600):
        step = 4
    commandTroops()
    if step < 4:
        if moveHero():
